...
def emp():
    try:
        conn = mysql.connect()
        cursor = conn.cursor(pymysql.cursors.DictCursor)
        cursor.execute("SELECT ProductoId , Nombre , TipoProductoId, Usuario, Estado  FROM cat_producto")
...
@app.route('/producto/<emp_id>')
def emp_details(emp_id):
    try:
        conn = mysql.connect()
        cursor = conn.cursor(pymysql.cursors.DictCursor)
        cursor.execute("SELECT ProductoId , Nombre , TipoProductoId, Usuario, Estado  FROM cat_producto WHERE ProductoId=%s",emp_id)
